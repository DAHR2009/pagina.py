...
fac = ["La mayoría de las personas que sufren adicción tecnológica experimentan un fuerte estrés cuando se encuentran fuera del área de cobertura de la red o no pueden utilizar sus dispositivos" ,
...
fun = ["Las tecnologias no tienen que ser malas , solo tienes que saber como utilizarlas y  saber cuando dejarlas" ,
"Aprende como utilizarlas y aprender junto a ellas"

]

@app.route("/")
def hello_world():
    return f'<h1>Dependencias electronicas</h1> <h2>Entra en el link y mira la informacion</h2> <a href="/fac">¡Ver un dato aleatorio!</a>'

@app.route("/fac")
def fac():
    return f'<p>{random.choice(fac)}</p>''<a href="/facs">¡Ver un dato aleatorio de la otra pagina!</a>'

@app.route("/facs")
def fac():
    return f'<p>{random.choice(fun)}</p>'
